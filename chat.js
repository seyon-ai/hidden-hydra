...
function startMsgListener(cid) {
  ...
  stopMsgs = onValue(msgRef, snap => {
    ...
    // Collect all messages from snapshot into ordered array
    const all = [];
    ...
    if (seenKeys.size === 0) {
      ...
      all.forEach((msg, i) => {
        ...
        seenKeys.add(msg._key);
        reactCache[msg._key] = JSON.stringify(msg.reactions || {});
      });
      wrap.scrollTop = wrap.scrollHeight;

    } else {
      // ── INCREMENTAL: only append brand-new messages ──
      const fresh = all.filter(m => !seenKeys.has(m._key));
      if (fresh.length) {
        fresh.forEach(msg => {
          wrap.appendChild(buildMsgEl(msg, false));
          seenKeys.add(msg._key);
          reactCache[msg._key] = JSON.stringify(msg.reactions || {});
        });
        wrap.scrollTop = wrap.scrollHeight;
      ...
      // ── REACTIONS: patch in-place only when changed ──
      all.forEach(msg => {
        const cur = JSON.stringify(msg.reactions || {});
        if (reactCache[msg._key] !== cur) {
          reactCache[msg._key] = cur;
          const row = wrap.querySelector(`[data-mid="${msg._key}"]`);
          if (row) {
            const rr = row.querySelector('[data-rr]');
            if (rr) drawReactions(rr, msg);
          }
        }
      });
    }
